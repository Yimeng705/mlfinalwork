...
class DiffuRecBase(object):
    ...
    def forward(self, feed_dict):
        ...
        if self.training:
            ...
            # 去噪（注意：DiffuRec只预测最后一个位置）
            denoised_seq = self.net(input_emb, x_t, self._scale_timesteps(t), mask_input, mask_target)
            ...
            # 评估模式：生成预测
            # 从随机噪声开始（只生成最后一个位置）
            noise_x_t = torch.randn_like(target_emb[:, -1:])  # [batch_size, 1, hidden_size]
            ...
            for i in indices:
                ...
                # 去噪
                denoised_seq = self.net(input_emb, noise_x_t, self._scale_timesteps(t), mask_input, mask_target)
...
class DiffuRecImpression(SequentialModel, DiffuRecBase):
